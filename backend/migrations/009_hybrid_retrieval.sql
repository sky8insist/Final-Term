...
create index if not exists content_blocks_search_vector_gin_idx
    on public.content_blocks using gin (search_vector);
...
language sql stable security invoker as $$
    select cb.id, cb.material_id, m.filename, cb.block_type, cb.content_text,
           ...
           ts_rank_cd(cb.search_vector, websearch_to_tsquery('simple', query_text)) as rank
    ...
    where cb.user_id = target_user_id
      ...
      and cb.search_vector @@ websearch_to_tsquery('simple', query_text)
