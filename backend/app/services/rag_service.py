...
def _build_prompt(*, question: str, raw_context: str, citations: list[dict], memory_snapshot: dict | None = None,
                  teaching_instruction: str | None = None, external_context: str = "") -> str:
    citation_lines = "\n".join(
...
助手记忆：
{memory_snapshot.get('assistant_memory', '')}
用户画像：
{memory_snapshot.get('user_profile', '')}
""".strip()
    return f"""
{_load_qa_prompt()}
...
async def answer_with_rag(*, user_id: str, subject_id: str, question: str, session_id: str | None = None,
                          teaching_instruction: str | None = None,
                          allow_external_knowledge: bool = False,
                          material_ids: list[str] | None = None,
                          block_types: list[str] | None = None) -> dict:
    # LightRAG's current synchronous adapter owns an event loop. Run the whole
    # blocking retrieval pipeline in a worker thread so async FastAPI handlers
    # never call asyncio.run() from their active loop.
    context = await asyncio.to_thread(
        search_subject_context, user_id=user_id, subject_id=subject_id, question=question,
        material_ids=material_ids, block_types=block_types,
    )
    citations = memory_service.normalize_retrieval_citations(context["citations"])
    external_context = ""
    material_is_insufficient = not context["rawContext"].strip() or not context["citations"]
    ...
    if (not context["rawContext"].strip() or not context["citations"]) and not external_context:
